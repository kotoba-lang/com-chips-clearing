(ns chips-clearing.main
  "Kotodama WASM entrypoint for the Chips_clearing clean-room actor (L4) — Clojure port.
  ...
  facts (`chips_clearing.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "chips_clearing")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"chips_clearing.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "chips_clearing-compat" :tier tier :entities entities} 200])
...
(defn start! [] :chips_clearing-compat/ready)
